...
var getSid = () => {
  ...
  var sid = window.location.hash.replace(/.*?sid=([^&]+)/, '$1');
  return sid || false;
};
...
class LoginWrapper extends _react.default.PureComponent {
  constructor() {
    ...
    _defineProperty(this, "handleLoginError", err => {
      this.setState({
        error: err,
        isLoading: false
      });
    ...
    _defineProperty(this, "exchangeToken", sid => {
      this.tokenExchange = _client.default.observable.request({
        uri: '/auth/exchange',
  ...
  componentDidMount() {
    ...
    if (sid) {
      ...
      var url = window.location.href.replace(`sid=${sid}`, '');
