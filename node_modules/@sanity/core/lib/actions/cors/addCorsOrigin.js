...
function filterOrigin(origin) {
  ...
  try {
    var example = origin.replace(/([^:])\*/g, `$1${wildcardReplacement}`).replace(/:\*/, portReplacement);
    ...
    var host = parsed.host.replace(/:(80|443)$/, '').replace(portReplacement, ':*').replace(new RegExp(wildcardReplacement, 'g'), '*');
