...
function createWeightedSearch(types, client) {
  ...
  var selections = searchSpec.map(spec => `_type == "${spec.typeName}" => {${spec.paths.map((cfg, i) => `"w${i}": ${cfg.path}`)}}`); // this is the actual search function that takes the search string and returns the hits

  return function search(queryString) {
    var opts = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    var terms = (0, _uniq2.default)((0, _compact2.default)((0, _words2.default)((0, _toLower2.default)(queryString))));
    var constraints = terms.map((term, i) => combinedSearchPaths.map(joinedPath => `${joinedPath} match $t${i}`));
    var filters = ['_type in $types', opts.includeDrafts === false && `!(_id in path('drafts.**'))`, ...constraints.map(constraint => `(${constraint.join('||')})`)].filter(Boolean);
    var query = `*[${filters.join('&&')}][0...$limit]{_type, _id, ...select(${selections.join(',\n')})}`;
